...
parse_args :: proc(
	...
	for os_arg, i in os_args[1:] {
		if os_arg[0] == '-' {
			...
			case 1:
				// only the flag found, no key-val
				flag_arg.flag = colon_slice[0]

			case 2:
				// key and/or value found
				flag_arg.flag = colon_slice[0]
			...
			case:
				// more than 1 colon found. Invalid syntax
				err = .Invalid_Format
				return
			}
			args[i] = flag_arg
...
builtin_flags := [?]Flag_Desc {
	{
		.Help,
		{"-help", "", ""},
		"Displays information about the build system or the target specified.",
	},
	...
	{.Dev, {"-build-pre-launch", "", ""}, "Runs the build system before debugging. (WIP)"},
	{
		...
		"Include the build system as a debugging target. (WIP)",
	},
	...
	{.Dev, {"-cwd-workspace", "", ""}, "Sets the CWD to the root of the build system executable."},
	{
		.Dev,
		{"-cwd-out", "", ""},
		"Sets the CWD to the output directory specified in the -out odin flag",
	},
	{
		.Dev,
		{"-launch-args", "<args>", ""},
		"The arguments to be sent to the output executable when debugging.",
	},
	{
		.Dev,
		{"-dbg", "<debugger name>", ""},
		"Debugger type used. Works with -vscode. Sets the ./vscode/launch.json \"type\" argument",
	},
}
...
print_general_help :: proc(info: Cli_Info) {
	...
	for target in info.project.targets {
		prefixed_name := strings.concatenate(
			{info.project.target_prefix, target.name},
...
run_cli :: proc(info: Cli_Info, cli_args: []string) -> bool {
	...
	target_names_from_args := make_dynamic_array_len_cap(
		[dynamic]string,
		0,
		len(info.project.targets),
		context.temp_allocator,
	) // This is not fully correct right now
	...
	for arg in args {
		...
		case Flag_Arg:
			...
			if !flag_found {
				fmt.eprintf(
					"Flag %s does not exist. Run `%s -help` for a list of available flags.\n",
					v.flag,
					cli_args[0],
				)
				return false
